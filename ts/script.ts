...
function toSecSinceMidnight(t: MyTime): number {
    return t.hour * 3600 + t.minute * 60 + t.second;
}

function fromSecSinceMidnight(t: number): MyTime {
...
function addOneSecond(t: MyTime): MyTime {
    return fromSecSinceMidnight(toSecSinceMidnight(t) + 1);
}

function addOneMinute(t: MyTime): MyTime {
    return fromSecSinceMidnight(toSecSinceMidnight(t) + 60);
}

function addOneHour(t: MyTime): MyTime {
    return fromSecSinceMidnight(toSecSinceMidnight(t) + 3600);
}

function addSeconds(t: MyTime, s: number): MyTime {
    return fromSecSinceMidnight(toSecSinceMidnight(t) + s);
}

function difference(mt1: MyTime, mt2: MyTime): number {
    return toSecSinceMidnight(mt1) - toSecSinceMidnight(mt2);
}

function isInRange(start: MyTime, finish: MyTime, t: MyTime): boolean {
...
function whatLesson(mt: MyTime): string {
